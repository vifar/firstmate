...
| Effort flag | `--thinking <off\|minimal\|low\|medium\|high\|xhigh\|max\|auto>`, a superset of the shared vocabulary; the selected model's own advertised thinking ladder can still omit a requested shared level, which `references/common/model-and-effort.md` owns. |
