...
description: >-
  ...
  Use whenever the session-start digest's bootstrap or network-checks section prints an actionable diagnostic line - MISSING, MISSING_MANUAL, PRESENTATION_UNAVAILABLE, BACKEND_INVALID, NEEDS_GH_AUTH, TANGLE, STARTUP_MEMORY_BUDGET, CREW_DISPATCH invalid, CREW_DISPATCH weak rules, TYPED_DISPATCH, FLEET_SYNC, NETWORK_CHECKS, HOME_SUMMARY, BACKLOG_RECONCILE, SECONDMATE_SYNC, SECONDMATE_LIVENESS, SECONDMATE_HANDOFF, NUDGE_SECONDMATES, or FMX - or reports that an interrupted backlog cleanup may have left an endpoint or local copy, or when a standalone bin/fm-bootstrap.sh or bin/fm-startup-network.sh run prints one of those lines.
...
- `CREW_DISPATCH: invalid config/crew-dispatch.json - <reason>` - the optional dispatch profile file exists but failed low-cost bootstrap validation; stop profile-based dispatch, report the actionable error, and require correction of the malformed schema, unverified harness name, or invalid harness/model/effort combination rather than falling back around it or selecting a bad profile.
- `CREW_DISPATCH: weak rules - typed resolve cannot discriminate (<reason>)` - `config/crew-dispatch.json` exists but cannot discriminate (fewer than two rules, or a single catch-all `when`); report the weak rules, require a discriminating rule set before relying on typed resolve, and keep using ordinary intake until the file can distinguish tasks.
- `TYPED_DISPATCH: off (TYPESAFE_API_KEY and AI_GATEWAY_API_KEY missing while config/crew-dispatch.json exists)` - dispatch profiles are active while typed resolution is unarmed; tell the captain that rule matching stays firstmate judgment until `TYPESAFE_API_KEY` or `AI_GATEWAY_API_KEY` is set in the environment or home `.env`, and that spawn will not enforce a resolve receipt until typed dispatch is armed.
