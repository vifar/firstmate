...
.env                 optional Relay pairing token (presence-gates section 14), mail-plane credentials (schema: docs/configuration.md "Mail plane"), and typed dispatch resolution keys TYPESAFE_API_KEY and/or AI_GATEWAY_API_KEY (presence-gates bin/fm-dispatch-resolve.sh; docs/configuration.md "Typed dispatch resolution"); LOCAL, gitignored
...
config/project-base-<project-name>  recorded integration base for fresh ship or scout worktrees when it differs from origin's default branch; LOCAL, gitignored, one token (the branch name), recorded during project intake, and NOT inherited by secondmate homes; absent or blank = origin's default branch. Resolution order: this file, then origin's default branch, owned by bin/fm-spawn.sh; see docs/configuration.md "Task launch base"
...
config/project-completion-contracts/<project>.md  optional project-specific single-line completion sentence rendered in place of the generic ship completion bar; LOCAL, gitignored, not inherited; see docs/configuration.md "Project completion contract"
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  <id>/dispatch-resolve  TOON receipt from bin/fm-dispatch-resolve.sh for this brief; required by fm-spawn when typed dispatch is armed
  <id>/dispatch-override.md  optional one-line reason when spawn profile deliberately differs from a clear receipt
...
state/               runtime records and signals; gitignored
  ...
  <id>.escalation.json  the structured prompt for one captain call, written by bin/fm-captain-hold.sh escalate and read back by prompt; owned entirely by that script, which retires it on every path that settles the call and sweeps departed ones with retire-escalations (docs/captain-hold-lifecycle.md)
...
Run `bin/fm-dispatch-resolve.sh` directly on the written brief in the same turn, with no preflight, write its full stdout TOON to `data/<id>/dispatch-resolve`, and on `clear` pass its `profile:` line to `fm-spawn` unless you write `data/<id>/dispatch-override.md` with a one-line reason; `ambiguous`, `escalate`, `error`, and off all mean the intake above, unchanged (contract: `docs/configuration.md` "Typed dispatch resolution"). When typed dispatch is armed, `fm-spawn` refuses a fresh crewmate or scout launch without that receipt.
...
For PR-based ship tasks, the worker's ready signal is a `done` line that independently reports the full current check set, pass/fail/pending counts, and unresolved review threads; before surfacing any merge decision, firstmate independently verifies the full current check and review-thread state with `bin/fm-pr-state.sh` and does not treat a worker claim as evidence.
...
The promoted worker follows the scratch-state, launch-base, branch, regression-test, and delivery instructions owned by `bin/fm-promote.sh`.
...
Every ordinary worker that is not authoritatively working must remain on an explicit lifecycle path: a declared bounded pause, a surfaced blocker or captain decision, safe recovery in its existing local copy, or guarded cleanup after a preserved terminal result.
`bin/fm-inactive-reconcile.sh` is the deterministic backstop for an inactive worker that otherwise falls out of that path; `stuck-crewmate-recovery` owns the response and never authorizes forced cleanup or discarded work.
...
A declared `paused:` event means a bounded external wait expected to clear on its own, while `blocked:` means firstmate action is needed. A declared wait is never proof that the worker has nothing to do: confirm the wait and also establish what the worker can do meanwhile (see `stuck-crewmate-recovery`).
...
- `ask-user-authority` - load before deciding any ask-user finding; it classifies findings as autonomous or requiring escalation.
- `captain-hold-lifecycle` - load before presenting or resolving any genuinely ambiguous, contract-expanding, destructive, irreversible, or security-sensitive choice; it owns the structured question and keyed answer lifecycle.
