...
That ownership proof is `fm_extension_owns_supervision` in `bin/fm-wake-lib.sh`, which accepts either the Pi pair (`fm_pi_extension_owns_supervision`) or the omp pair (`fm_omp_extension_owns_supervision`): both primary extensions of one family must have state markers binding their current on-disk builds to the process named in `state/.lock`, and that process must still be alive; omp never inherits the Pi tolerance because its proof is keyed on its own two files and markers.
For omp's nested extension workers, `bin/fm-session-start.sh` binds existing current-build loader markers to their verified session-lock ancestor after lock acquisition and before startup diagnostics, for both native startup and agent-driven resume/continue startup; it never creates missing extension evidence or adopts an unrelated loader.
