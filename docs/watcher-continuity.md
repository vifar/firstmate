...
omp's replacement follows the same generation-owner contract in `.omp/extensions/fm-primary-omp-watch.ts`, whose header owns the two differences: omp reports no shutdown reason, so a shutdown that still holds a pending actionable close persists the handoff for the next owning `session_start` to replay, and that handoff carries only closes still fresh enough to be the pending close they were written for, so a `session_start` can never inherit a replay backlog of finished work.
