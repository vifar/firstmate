...
omp's replacement follows the same generation-owner contract in `.omp/extensions/fm-primary-omp-watch.ts`; its header owns omp's shutdown persistence plus the eligibility and replay bounds that prevent a `session_start` from inheriting finished work.
