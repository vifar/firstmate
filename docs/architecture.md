...
So a delivered ordinary crew task whose last line stays `done: PR ...` bounds repeated alarms from new pane hashes to the `FM_PAUSE_RESURFACE_SECS` cadence for the length of the captain's decision, and a held task whose last line is `working:` or another non-declaration uses that same cadence when its stale timer reaches the escalation threshold.
...
A secondmate reaches the stale path only for a wait declared in its status line, so a hold recorded only in the backlog while its last line is `working:` or `done:` remains outside this guard.
That secondmate exception deliberately avoids backlog reads on the ordinary poll hot path; ordinary primary crew tasks consult the backlog only in the at-threshold branch that is about to escalate.
...
Which verb declared it decides how the recheck is worded, because the two block on different people: a `paused:` declaration is owed by an external dependency the worker named, so its recheck asks the reader to confirm that dependency AND to establish what the worker could do while it waits, because a valid blocker can coexist with a worker that has plenty of available work; a hold is owed by the captain reading the recheck and asks them to answer the held decision or release the hold.
Wording a hold as an external wait would point the captain away from the one action that clears it, and wording a pause as a re-validation of its blocker alone can never detect the idle-but-unblocked worker the pause is most likely to hide.
...
Inactive `blocked`, `parked`, `paused`, or `unknown` results instead produce one durable actionable notification per task incarnation and exact obligation fingerprint, naming the status-ledger reason; an unchanged obligation stays quiet after presentation, while a changed blocker, decision, declared wait, or unreadable verdict re-surfaces.
A `paused` obligation is not a wedge alarm and does not shorten the declared wait's bounded recheck cadence; it exists because the cadence can only ask whether the declared blocker still holds, and a valid blocker can coexist with a worker that has plenty of available work, so the notification carries that second question too.
Working workers remain active, and captain-held work retains its existing bounded wait semantics.
...
Away mode is a posture of the one supervision session, recorded in `state/.afk-contract` by `bin/fm-afk-contract.sh` after the captain confirms a plain-sentence read-back of their away words, and announced at entry as hold-for-return only because no phone channel exists.
...
When typed dispatch is armed, `bin/fm-dispatch-resolve.sh` supplies that rule match as one short tool turn and `fm-spawn.sh` requires the persisted receipt; [`configuration.md`](configuration.md#typed-dispatch-resolution-env-typesafe_api_key--ai_gateway_api_key) owns that opt-in contract.
The session-start bootstrap step keeps schema-valid dispatch configuration silent unless verbose facts or the typed-dispatch G1/G2 diagnostics fire, and surfaces a concise invalid-config line when validation fails.
