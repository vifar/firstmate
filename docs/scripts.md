...
| `fm-session-lock-lib.sh` | Shared session-lock harness identity (ancestry walk, holder liveness, and Claude foreign-owner detection) for fm-lock.sh, the Claude Stop auto-arm, and the Claude turn-end guard |
...
| `fm-inactive-reconcile.sh` | Reconcile long-inactive terminal outcomes and attempt safe main-home cleanup; [header contract](../bin/fm-inactive-reconcile.sh) |
...
| `fm-pr-state.sh`         | Read-only: enumerate GitHub's complete current check rollup and unresolved review threads, with counts and blockers |
...
| `fm-harness.sh`          | Detect the running harness, resolve crew or secondmate harness, model, and effort, own the model-aware effort-ladder lookup (`effort-verdict`/`effort-verdicts`), and validate the native-only `ultra` effort |
