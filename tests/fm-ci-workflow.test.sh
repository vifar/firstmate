...
test_ci_registers_pull_request_checks() {
  ruby -ryaml -e '
workflow = YAML.load_file(ARGV[0])
events = workflow.fetch(true) { workflow.fetch("on") }
pull_request = events.fetch("pull_request")
raise "CI must run for pull requests targeting main" unless pull_request.fetch("branches") == ["main"]
raise "CI must run on main pushes" unless events.fetch("push").fetch("branches") == ["main"]
raise "CI must grant read-only repository access" unless workflow.fetch("permissions") == {"contents" => "read"}
' "$CI_WORKFLOW" || fail "CI workflow must register real pull_request checks without changing push coverage or permission boundaries"
  pass "CI registers pull-request and main-push checks with read-only permissions"
}
